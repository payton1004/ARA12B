...
mf = pd.concat(map(functools.partial(pd.read_csv, delimiter=','), 
            sorted(glob.glob('/Users/jung-ok/work1/'+cruise+'/processed/'+folder_name+'/'+subfolder_name+'/'+'chl'+'/anom04/'+'2021*.csv')))) 

_file = '/Users/jung-ok/work1/'+cruise+'/processed/'+folder_name+'/'+subfolder_name+'/'+'chl'+'/anom04/'+'chl_flag.csv'
...
def plot_days(day1: str, df: pd.DataFrame):
    ...
    # fname=input('fname = ')
    _file = '/Users/jung-ok/work1/'+cruise+'/plot/'+folder_name+'/'+subfolder_name+'/'+'chl'+'/anom04/'+fname
