...
data='/Users/jung-ok/work1/ARA12B/stationinfo/ARA12B_station_lat_long_20210731.csv'
...
# ### DIC 채수 
# # gmt psxy -R -J -Sc0.20 -Gyellow -Wfaint,black  -O -V -K <<EOF>> $ps
# # -167.89827 74.800615  # 40
# # EOF

# # No. 1, St.023
# awk -F, '{if (NR==24) print $3, $2}' $data | 
# gmt psxy -R -J -Sc0.20 -Gyellow -Wfaint,black  -O -V -K >> $ps

# # No. 2, St.033
# awk -F, '{if (NR==34) print $3, $2}' $data | 
# gmt psxy -R -J -Sc0.20 -Gyellow -Wfaint,black  -O -V -K >> $ps

# # No. 3, St.040
# awk -F, '{if (NR==41) print $3, $2}' $data | 
# gmt psxy -R -J -Sc0.20 -Gyellow -Wfaint,black  -O -V -K >> $ps

# # No. 4, St.041
# awk -F, '{if (NR==42) print $3, $2}' $data | 
# gmt psxy -R -J -Sc0.20 -Gyellow -Wfaint,black  -O -V -K >> $ps

# # No. 5, St.045
# awk -F, '{if (NR==46) print $3, $2}' $data | 
# gmt psxy -R -J -Sc0.20 -Gyellow -Wfaint,black  -O -V -K >> $ps
...
# # No. 7, St.056  
# awk -F, '{if (NR==57) print $3, $2}' $data | 
# gmt psxy -R -J -Sc0.20 -Gyellow -Wfaint,black  -O -V -K >> $ps

# # No. 8, St.081  
# awk -F, '{if (NR==85) print $3, $2}' $data | 
# gmt psxy -R -J -Sc0.20 -Gyellow -Wfaint,black  -O -V -K >> $ps

# # No. 9, St.087  
# awk -F, '{if (NR==91) print $3, $2}' $data | 
# gmt psxy -R -J -Sc0.20 -Gyellow -Wfaint,black  -O -V -K >> $ps

# # No. 10, St.95  
# awk -F, '{if (NR==99) print $3, $2}' $data | 
# gmt psxy -R -J -Sc0.20 -Gyellow -Wfaint,black  -O -V -K >> $ps
...
# ### CTD DO 채수 
# # gmt psxy -R -J -Sc0.07 -Gblue -Wfaint,black  -O -V -K <<EOF>> $ps
# # -167.89827 74.800615  # 23
# # EOF

# # No. 1, St.023
# awk -F, '{if (NR==24) print $3, $2}' $data | 
# gmt psxy -R -J -Sc0.07 -Ggray -Wfaint,black  -O -V -K >> $ps

# # No. 2, St.034
# awk -F, '{if (NR==35) print $3, $2}' $data | 
# gmt psxy -R -J -Sc0.07 -Ggray -Wfaint,black  -O -V -K >> $ps
...
# # 41
# awk -F, '{if (NR==42) print $3, $2, $1}' $data | 
# gmt pstext -R -J  -F+f4p,Helvetica-Bold,black+jCM  -V  -O -K -D-0.14/0.06  >> $ps

# # 42 ~ 51
# awk -F, '{if (NR>42 && NR<=52) print $3, $2, $1}' $data | 
# gmt pstext -R -J  -F+f4p,Helvetica-Bold,black+jCM  -V  -O -K -D0.14/0.06  >> $ps

# # 52 ~ 56
# awk -F, '{if (NR>52 && NR<=57) print $3, $2, $1}' $data | 
# gmt pstext -R -J  -F+f4p,Helvetica-Bold,black+jCM  -V  -O -K -D-0.14/0.06  >> $ps

# # 57
# awk -F, '{if (NR==58) print $3, $2, $1}' $data | 
# gmt pstext -R -J  -F+f4p,Helvetica-Bold,black+jCM  -V  -O -K -D-0.14/-0.06  >> $ps

# # WP1
# awk -F, '{if (NR==59) print $3, $2, $1}' $data | 
...
# # 58H
# awk -F, '{if (NR==61) print $3, $2, $1}' $data | 
# gmt pstext -R -J   -F+f3p,Helvetica-Bold,dimgray+jCM   -V  -O -K -D0.2/0.05  >> $ps

# # 58I
# awk -F, '{if (NR==62) print $3, $2, $1}' $data | 
# gmt pstext -R -J   -F+f3p,Helvetica-Bold,dimgray+jCM   -V  -O -K -D-0.14/0.06  >> $ps

# # 59 ~ 60
# awk -F, '{if (NR>62 && NR<=64) print $3, $2, $1}' $data | 
# gmt pstext -R -J  -F+f4p,Helvetica-Bold,black+jCM  -V  -O -K -D-0.13/0.06  >> $ps

# # 61 ~ 62
# awk -F, '{if (NR>64 && NR<=66) print $3, $2, $1}' $data | 
# gmt pstext -R -J  -F+f4p,Helvetica-Bold,black+jCM  -V  -O -K -D-0.0/0.13  >> $ps

# # 63 ~ 65
# awk -F, '{if (NR>66 && NR<=69) print $3, $2, $1}' $data | 
# gmt pstext -R -J  -F+f4p,Helvetica-Bold,black+jCM  -V  -O -K -D0./-0.13  >> $ps

# # 66 ~ 69
# awk -F, '{if (NR>69 && NR<=73) print $3, $2, $1}' $data | 
# gmt pstext -R -J  -F+f4p,Helvetica-Bold,black+jCM  -V  -O -K -D-0./-0.13  >> $ps

# # 70
# awk -F, '{if (NR==74) print $3, $2, $1}' $data | 
# gmt pstext -R -J  -F+f4p,Helvetica-Bold,black+jCM  -V  -O -K -D-0.15/-0.  >> $ps

# # 71
# awk -F, '{if (NR==75) print $3, $2, $1}' $data | 
# gmt pstext -R -J  -F+f4p,Helvetica-Bold,black+jCM  -V  -O -K -D-0./0.13  >> $ps

# # 72
# awk -F, '{if (NR==76) print $3, $2, $1}' $data | 
# gmt pstext -R -J  -F+f4p,Helvetica-Bold,black+jCM  -V  -O -K -D-0.15/-0.  >> $ps

# # 73 ~ 84
# awk -F, '{if (NR>76 && NR<=88) print $3, $2, $1}' $data | 
# gmt pstext -R -J  -F+f4p,Helvetica-Bold,black+jCM  -V  -O -K -D-0.14/-0.06  >> $ps

# # 85 ~ 90
# awk -F, '{if (NR>88 && NR<=94) print $3, $2, $1}' $data | 
# gmt pstext -R -J  -F+f4p,Helvetica-Bold,black+jCM  -V  -O -K -D0.14/-0.06  >> $ps

# # 91 ~ 92
# awk -F, '{if (NR>94 && NR<=96) print $3, $2, $1}' $data | 
# gmt pstext -R -J  -F+f4p,Helvetica-Bold,black+jCM  -V  -O -K -D0./-0.13  >> $ps

# # 93
# awk -F, '{if (NR==97) print $3, $2, $1}' $data | 
# gmt pstext -R -J  -F+f4p,Helvetica-Bold,black+jCM  -V  -O -K -D-0./-0.13  >> $ps

# # 94
# awk -F, '{if (NR==98) print $3, $2, $1}' $data | 
# gmt pstext -R -J  -F+f4p,Helvetica-Bold,black+jCM  -V  -O -K -D-0./-0.13  >> $ps

# # 95
# awk -F, '{if (NR==99) print $3, $2, $1}' $data | 
